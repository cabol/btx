defmodule BTx.JRPC.Wallet.GetNewAddressTest do
  ...
  alias BTx.JRPC.Wallets.{CreateWallet, GetNewAddress}
  ...
  describe "integration with BTx.JRPC" do
    ...
    test "successful call returns new address", %{
      ...
      assert {:ok, response} = JRPC.call(client, method)
    ...
    test "call with default values", %{client: client, url: url, wallet_name: wallet_name} do
      method = GetNewAddress.new!(wallet_name: wallet_name)
    ...
    test "call with only label specified", %{client: client, url: url, wallet_name: wallet_name} do
      method = GetNewAddress.new!(label: "savings", wallet_name: wallet_name)
    ...
    test "call with only address_type specified", %{
      ...
      method = GetNewAddress.new!(address_type: "legacy", wallet_name: wallet_name)
    ...
    test "handles RPC error response", %{client: client, url: url, wallet_name: wallet_name} do
      method = GetNewAddress.new!(label: "test", wallet_name: wallet_name)
    ...
    test "verifies all address types work", %{client: client, url: url, wallet_name: wallet_name} do
      ...
      for {address_type, prefix} <- Enum.zip(address_types, expected_prefixes) do
        method =
          GetNewAddress.new!(label: "test", address_type: address_type, wallet_name: wallet_name)
    ...
    test "real Bitcoin regtest integration" do
      ...
      assert {:ok, response} =
               JRPC.call(
                 real_client,
                 GetNewAddress.new!(wallet_name: wallet_name)
               )
