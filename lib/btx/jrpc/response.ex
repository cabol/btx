defmodule BTx.JRPC.Response do
  @moduledoc """
  ...
  """
  @spec new(Tesla.Env.t()) :: {:ok, t()} | {:error, BTx.JRPC.MethodError.t() | BTx.JRPC.Error.t()}
  def new(response)
  ...
  def new(%Tesla.Env{
    ...
    wrap_error BTx.JRPC.MethodError, id: body["id"], code: code, message: message
  end

  # Fallback for any other unexpected cases
  def new(%Tesla.Env{status: status, body: body}) do
    wrap_error BTx.JRPC.Error, reason: {:rpc, :unknown_error}, status: status, body: body
  end
end
